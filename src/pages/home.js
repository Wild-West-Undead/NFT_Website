...
useEffect(() => {
		...
		if (!window.ethereum) {
			seterrMsg('install metamask extension!!')
	...
	const checkWalletBalance = async (account) =>{
		...
		if (checkNetworkIdValid !== '4') {
			seterrMsg('Please Switch to Rinkeby Test Network')
			return ;
		}
	...
	const mintNftHandler = async () => {
		try {
			...
			const account = await window.ethereum
				...
				let CurrentNftPriceBN = await nftContract.WLPRICE();
				let CurrentNftPrice = ethers.utils.formatEther(CurrentNftPriceBN.toString())
				let amount = 1;
				let nftPrice = CurrentNftPrice * amount;
				let nftPrice1 = (100000000000000 * amount).toString();
				let t1 = (1000 * nftPrice);
				let nftPrice2 = (1000000000000000 * t1).toString()
				let balance = BigNumber.from(nftPrice2.toString());

				let isActive = await nftContract.saleIsActive.call();
				const activeBadgeId = ethers.BigNumber.from(
					await nftContract.activeBadgeId()
				).toNumber();
				...
				if(ethers.utils.formatEther(tempBalance) < nftPrice) {
					seterrMsg('Not enough ETH for transaction')
					alert('Not enough ETH for transaction');
					return;
				}
				if (balanceOf >= maxPerWallet) {
					seterrMsg('Max tkn per wallet exceeded')
					alert('Max tkn per wallet exceeded');
					return;
				}
				const nftTxn = await nftContract.PUBLICMINT(amount, { value: ethers.utils.parseUnits( balance.toString(),'wei') });
				setLoader(true);
			...
			{
			 alert('Something went wrong or user cancelled transaction')
			}
					...
					<BannerBX>
						<BannerSBX01>
							...
							</BTNBX02>
							{
								...
								<button
									onClick={() => {
										window.open(`https://testnets.opensea.io/assets/${chainID}/${nftContract.address}/${activeBadgeId}`)
									}}
								>
									View Tokens
								</button>
